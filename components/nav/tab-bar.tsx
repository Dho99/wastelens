    ...
    return (
        <div className="flex min-h-screen flex-col max-w-screen-sm m-auto w-full bg-[#FAF9F5]">
            ...
            <nav className="fixed inset-x-0 bottom-0 z-50 bg-transparent max-w-screen-sm mx-auto w-full px-4 pb-4 pointer-events-none">
                <div
                    className={`bg-white rounded-t-4xl rounded-b-3xl items-end flex p-2 pointer-events-auto border border-gray-100/50 shadow-xl`}
                >
                    {tabs.map((tab, index) => {
                        return tab.primaryMenu ? (
                            ...
                            <Link
                                key={index}
                                href={tab.href}
                                className={`flex-1 flex flex-col items-center gap-1 font-medium transition-all duration-200 p-2 ${
                                    pathname === `${tab.href}`
