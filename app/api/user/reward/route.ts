...
export async function GET(request: NextRequest) {
  try {
    ...
    const [userData, earnedThisWeek, recentPenukaran, kopdesList] = await Promise.all([
    ...
    const partners = kopdesList.map((k) => ({
      id: k.id,
      name: k.nama,
      distance: isNaN(lat) || isNaN(lng) ? "" : `${Math.round(Math.random() * 2 + 0.1 * 10) / 10}km`,
    ...
    const history = recentPenukaran.map((p) => ({
      ...
      status: p.status === "REDEEMED" ? "BERHASIL" as const : p.status === "PENDING" ? "PROSES" as const : "GAGAL" as const,
