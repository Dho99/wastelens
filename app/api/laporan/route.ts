...
import { createReport, ReportError } from "@/server/modules/reports/report.service";
...
async function handleLegacyPayload(body: Record<string, unknown>, userId: string) {
  const { image, lat, lng, kategori_ukuran, rekomendasi_kendaraan } = body as {
    image?: string;
    lat?: unknown;
    lng?: unknown;
  ...
  triggerUserEvent(
    userId,
    createReportCreatedEvent({ laporanId: result.id, status: result.status }),
  );
