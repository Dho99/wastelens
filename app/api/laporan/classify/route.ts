...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!upload) {
      return NextResponse.json(
        { error: "File temporary tidak ditemukan", code: "TEMP_FILE_NOT_FOUND" },
        { status: 404 },
      );
    }

    const result = await analyzeWasteImage(upload.secure_url, upload.mime_type);
    ...
    return NextResponse.json({ error: message, code: "AI_SERVICE" }, { status: 502 });
  }
}
