        ...
        <div className="space-y-6 py-4">
          ...
          <div className="relative overflow-hidden rounded-xl bg-primary p-5 text-white shadow-sm">
            <div className="relative z-10">
              ...
              <h2 className="mb-1 text-3xl font-bold leading-tight">0 Lokasi</h2>
        ...
        <section className="space-y-3">
          ...
          <div className="relative overflow-hidden rounded-xl bg-primary p-5 text-white shadow-sm">
            <div className="relative z-10">
              ...
              <p className="text-[13px] font-medium text-emerald-50">
                {selesai_hari_ini} Selesai &bull; {tersisa} Tersisa
              </p>
            </div>
            <Icon
              path={mdiClipboardText}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col justify-center rounded-xl border border-neutral-300 bg-primary/10 p-4">
              <Icon path={mdiMapMarkerPath} className="mb-2 h-5 w-5 text-neutral-500" />
              <p className="mb-0.5 text-[11px] font-medium text-neutral-500">
                Total Tugas
              </p>
              <p className="text-[17px] font-bold text-[#205c48]">{total_tasks}</p>
            </div>
            <div className="flex flex-col justify-center rounded-xl border border-neutral-300 bg-primary/10 p-4">
              <Icon path={mdiTimerOutline} className="mb-2 h-5 w-5 text-accent" />
              <p className="mb-0.5 text-[11px] font-medium text-neutral-500">
                Selesai Hari Ini
              </p>
              <p className="text-[17px] font-bold text-accent">{selesai_hari_ini}</p>
            </div>
          </div>
        </section>
        ...
        <section>
          <div className="mb-3 flex items-center justify-between px-1">
            ...
            <button className="flex items-center gap-1 text-[13px] font-semibold text-[#388e3c]">
              Urutkan <Icon path={mdiFilterVariant} className="h-4 w-4" />
            </button>
          </div>

          <div className="space-y-4">
            {recent_tasks.map((task, i) => (
              <TaskCard key={task.id} task={task} first={i === 0} router={router} />
            ))}
          </div>
        </section>
      </div>
  ...
  return (
    ...
    >
        ...
        <div className="relative h-25 w-25 shrink-0 overflow-hidden rounded-xl">
          {isHighPriority && (
            <span className="absolute left-0 top-0 z-10 flex items-center gap-1 rounded-br-lg bg-[#d32f2f] px-1.5 py-0.5 text-[9px] font-bold text-white">
              <Icon path={mdiAlertOutline} className="h-2.5 w-2.5" /> Prioritas
              Tinggi
            </span>
          )}
          <Image
      ...
      <div className="px-3 pb-3 pt-1">
        ...
        >
          Mulai Tugas <Icon path={mdiChevronRight} className="h-4 w-4" />
        </button>
      </div>
    </div>
  );
