...
export function ReportsManagement() {
  ...
  const setReports = (update: (current: Report[]) => Report[]) =>
    updateDlhStore((draft) => {
      draft.reports = update(draft.reports);
    });
  ...
  const filtered = useMemo(
    () =>
      reports.filter((report) => {
        ...
        const searchMatch = `${report.id} ${report.location} ${report.district}`
          .toLowerCase()
          .includes(term);
  ...
  const confirmDelete = () => {
    ...
    setReports((current) =>
      current.filter((report) => report.id !== deletedId),
    );
  ...
  const exportCsv = () => {
    const source = selectedIds.length
      ? filtered.filter((report) => selectedIds.includes(report.id))
      : filtered;
    const rows = [
      "ID,Tanggal,Waktu,Lokasi,Kategori,Status",
      ...source.map(
        (report) =>
          `${report.id},${report.date} ${report.year},${report.time},${report.location},${report.category},${report.status}`,
      ),
    ];
        ...
        <div className="mx-auto max-w-[1400px]">
          ...
          <section className="mt-6 flex flex-col gap-4 rounded-[24px] border border-[#b7cbbd] bg-white/55 p-4 lg:flex-row lg:items-center">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <span className="mr-1 text-xs font-bold text-[#46594f]">
                Filter Status:
              </span>
              {(["Semua", "Menunggu", "Diproses", "Selesai"] as const).map(
                (item) => (
                  <button
                    type="button"
                    key={item}
            ...
            <label className="relative flex h-11 items-center gap-2 rounded-full bg-[#e8f6fd] px-4 text-xs font-bold text-[#536159]">
          ...
          <section className="mt-6 overflow-hidden rounded-[24px] border border-[#b7cbbd] bg-white/35">
                  ...
                  {visibleReports.map((report) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-6">
                        ...
                        <p className="text-xs text-[#68756e]">{report.time}</p>
                      </td>
                      <td className="px-4 py-6">
                      ...
                      <td className="px-4 py-6">
                        <span
                          className={`inline-block min-w-20 rounded-full px-3 py-1 text-center text-[10px] font-extrabold ${report.category === "BAHAYA" ? "bg-red-600 text-white" : "bg-[#bcebd1] text-[#47705b]"}`}
                        >
                          {report.category}
                        </span>
                      </td>
                      <td className="px-4 py-6">
                        <span
                          className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-bold ${statusStyles[report.status]}`}
                        >
                          <span
                            className={`size-2 rounded-full ${report.status === "Selesai" ? "bg-[#087529]" : "bg-[#698174]"}`}
                          />
                          {report.status}
                        </span>
                      </td>
                      <td className="px-4 py-6">
                        <div className="flex justify-center gap-4">
                          <button
                            type="button"
                            onClick={() =>
                              router.push(`/dinas/reports/${report.id}`)
                            }
            ...
            <footer className="flex flex-col gap-4 border-t border-[#becdbf] px-6 py-4 text-xs text-[#536159] sm:flex-row sm:items-center">
              <p>
                Menampilkan {filtered.length ? (page - 1) * perPage + 1 : 0}–
                {Math.min(page * perPage, filtered.length)} dari{" "}
                {filtered.length || 0} laporan
              </p>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            <div className="flex items-center gap-4 rounded-[22px] border border-[#b7cbbd] bg-[#e7f5fc] p-5">
            ...
            <div className="flex items-center gap-4 rounded-[22px] border border-[#b7cbbd] bg-[#e7f5fc] p-5">
              ...
              <div>
                ...
                <p className="text-2xl font-extrabold">
                  {
                    reports.filter((report) => report.status === "Diproses")
                      .length
                  }
                </p>
              </div>
            ...
            <div className="flex items-center gap-4 rounded-[22px] border border-[#b7cbbd] bg-[#e7f5fc] p-5">
              ...
              <div>
                ...
                <p className="text-2xl font-extrabold">
                  {
                    reports.filter((report) => report.status === "Selesai")
                      .length
                  }
                </p>
              </div>
        ...
        >
          ...
          >
            ...
            <h2
              ...
              className="mt-5 text-lg font-extrabold text-[#26312a]"
            >
              Hapus laporan?
            </h2>
            <p className="mx-auto mt-2 max-w-[310px] text-sm leading-relaxed text-[#667169]">
              Laporan{" "}
              <span className="font-bold text-[#c51c21]">
                #{pendingDeleteId}
              </span>{" "}
      ...
      {deletedReportId && (
        ...
        >
          ...
          >
            ...
            >
              Data laporan{" "}
              <span className="text-[#c51c21]">#{deletedReportId}</span> telah
              dihapus
              <br className="hidden sm:block" /> permanen dari sistem
              operasional.
            </p>
            <button
              type="button"
              autoFocus
              onClick={() => setDeletedReportId(null)}
