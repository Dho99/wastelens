...
function LocationMap() {
  return (
    <div className="relative h-[230px] overflow-hidden bg-[#d9f1f7] sm:h-[270px]">
      ...
      <span className="absolute left-[9%] top-[18%] rotate-[-6deg] text-xs font-bold text-[#6c8790]">
        Jl. Kebon Sirih
      </span>
      <span className="absolute bottom-[18%] right-[12%] rotate-[3deg] text-xs font-bold text-[#6c8790]">
        Jl. M.H. Thamrin
      </span>
      <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-sm font-extrabold text-[#48715f]">
        MENTENG
      </span>
      <span className="absolute left-[56%] top-[47%] grid size-10 place-items-center rounded-full border-4 border-white bg-[#087529] text-white shadow-lg">
...
export function ReportDetail({ reportId }: { reportId: string }) {
  ...
  const report =
    store.reports.find((item) => item.id === reportId) ?? store.reports[0];
  const detail = {
      ...
      <main className="min-h-0 flex-1 overflow-y-auto bg-[#f4fbff] px-4 pb-10 pt-8 sm:px-7 lg:px-10">
        <div className="mx-auto max-w-[1180px]">
          ...
          <section className="mt-8 grid gap-5 rounded-[20px] border border-[#bdcdbf] bg-white px-6 py-5 shadow-sm sm:grid-cols-3">
            <div>
              <p className="text-xs font-semibold text-[#7a877f]">Pelapor</p>
              <p className="mt-1 text-lg font-extrabold">{detail.reporter}</p>
            </div>
            <div>
              <p className="text-xs font-semibold text-[#7a877f]">
                Jam Laporan
              </p>
              <p className="mt-1 text-lg font-extrabold">{detail.time}</p>
            </div>
            <div>
              <p className="text-xs font-semibold text-[#7a877f]">Alamat</p>
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,2.1fr)_minmax(280px,0.95fr)]">
            <section className="overflow-hidden rounded-[22px] border border-[#bdcdbf] bg-white shadow-sm">
              <div className="flex h-14 items-center gap-2 bg-[#e7f6fd] px-5">
                ...
                <h2 className="text-lg font-extrabold">Foto Laporan Warga</h2>
              </div>
                ...
                {timeline.map(([label, time], index) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#7a877f]">{time}</p>
                    </div>
                  </li>
                ))}
              </ol>
          ...
          <section className="mt-7 overflow-hidden rounded-[22px] border border-[#bdcdbf] bg-white shadow-sm">
            <div className="flex h-14 items-center gap-2 bg-[#e7f6fd] px-5">
              ...
              <h2 className="text-lg font-extrabold">Koordinat Lokasi</h2>
            </div>
            <LocationMap />
            <div className="px-5 py-4">
              <p className="font-extrabold">{detail.address}</p>
              <p className="mt-1 text-xs text-[#66746c]">{detail.district}</p>
            </div>
          </section>
          ...
          >
            <div className="flex items-center">
              <div>
                ...
                <h2 className="mt-1 text-xl font-extrabold">#{reportId}</h2>
              </div>
              <button
                type="button"
