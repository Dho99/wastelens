...
const vehicleSeed: DlhVehicle[] = Array.from({ length: 10 }, (_, index) => {
  ...
  return {
    ...
    status:
      index === 1 || index === 4 || index === 7 ? "Maintenance" : "Beroperasi",
    ...
    maintenance: [
      {
        ...
        date: "2024-05-12",
        description: "Ganti oli mesin, pemeriksaan hidrolik, dan rotasi ban.",
        status: "Selesai",
      },
      {
        id: `M-${number}-2`,
...
function clientSnapshot() {
  if (!initialized && typeof window !== "undefined") {
    ...
    try {
      ...
      if (stored)
        currentSnapshot = { ...defaultDlhState, ...JSON.parse(stored) };
    } catch {
