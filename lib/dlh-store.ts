...
const reportSeed: DlhReport[] = [
    ...
    (
        ...
        assignedVehicleId:
            status === "Menunggu"
                ? undefined
                : `ARM-${String((index % 10) + 1).padStart(3, "0")}`,
        assignedOfficerId:
            status === "Menunggu" ? undefined : `FLD-${9921 + (index % 10)}`,
    }),
);
        ...
        maintenance: [
            {
                ...
                date: "2024-05-12",
                description:
                    "Ganti oli mesin, pemeriksaan hidrolik, dan rotasi ban.",
                status: "Selesai",
            },
            {
                id: `M-${number}-2`,
                title: "Uji Emisi Kendaraan (KIR)",
                date: "2024-03-15",
                description: "Sertifikasi KIR berkala dan pemeriksaan emisi.",
                status: "Selesai",
            },
        ],
    };
...
const officerSeed: DlhOfficer[] = officerNames.map((name, index) => ({
    ...
    color: ["bg-[#bcebd1]", "bg-[#ffd9ae]", "bg-[#d7eafd]", "bg-[#e5ddfb]"][
        index % 4
    ],
    role: "Field Operator",
    shift: "Pagi",
    ...
    recentTasks: [
        {
            id: `T-${index}-1`,
            place: "Jl. Imam Bonjol No. 44",
            time: "Selesai 15 menit lalu",
            note: "Pengangkutan berhasil dan area telah dibersihkan.",
        },
    ],
}));
    ...
    notifications: [
        {
            ...
            message: "Laporan #WL-099 di Menteng membutuhkan armada besar.",
            time: "8 menit lalu",
            type: "report",
            read: false,
        },
        {
            ...
            message: "Truk Sampah 02 telah tiba untuk laporan #WL-098.",
            time: "18 menit lalu",
            type: "truck",
            read: false,
        },
        {
            ...
            message: "Petugas menyelesaikan laporan #WL-096.",
            time: "1 jam lalu",
            type: "done",
            read: true,
        },
...
function normalizeState(value: Partial<DlhState>): DlhState {
    return {
        ...
        reports: Array.isArray(value.reports)
            ? value.reports
            : defaultDlhState.reports,
        vehicles: Array.isArray(value.vehicles)
            ? value.vehicles
            : defaultDlhState.vehicles,
        officers: Array.isArray(value.officers)
            ? value.officers
            : defaultDlhState.officers,
        accounts: Array.isArray(value.accounts)
            ? value.accounts
            : defaultDlhState.accounts,
        notifications: Array.isArray(value.notifications)
            ? value.notifications
            : defaultDlhState.notifications,
    };
}
...
function scheduleBackendPersist() {
    ...
    persistTimer = window.setTimeout(() => {
        persistTimer = null;
        persistenceChain = persistenceChain.then(
            persistToBackend,
            persistToBackend,
        );
    }, 300);
...
function clientSnapshot() {
    if (!initialized && typeof window !== "undefined") {
        ...
        try {
            ...
            if (stored) {
                const parsed = JSON.parse(stored) as Partial<DlhState> & {
                    schemaVersion?: number;
                };
                const reports = parsed.reports?.map((report, index) =>
                    parsed.schemaVersion === 2 || report.status === "Menunggu"
                        ? report
                        : {
                              ...report,
                ...
                currentSnapshot = normalizeState({
                    ...parsed,
                    reports: reports ?? defaultDlhState.reports,
                });
...
export function updateDlhStore(update: (draft: DlhState) => void) {
    ...
    try {
        ...
        throw new Error(
            "Penyimpanan lokal penuh. Gunakan foto yang lebih kecil.",
        );
    }
    listeners.forEach((listener) => listener());
