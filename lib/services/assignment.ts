...
function computeEstimatedLoadKg(kategoriUkuran: string): number {
  return LOAD_ESTIMATES_KG[kategoriUkuran.toUpperCase()] ?? LOAD_ESTIMATES_KG.UNCERTAIN;
}
...
export async function assignKendaraan(
...
): Promise<{ kendaraan_id: string | null; petugas_id: string | null; estimatedLoadKg: number }> {
  ...
  const available = await prisma.$queryRawUnsafe<Array<{ id: string; current_load: number; kapasitas: number }>>(
  ...
  if (!available || available.length === 0) {
    return { kendaraan_id: null, petugas_id: null, estimatedLoadKg: estimatedLoad };
  }
...
export async function assignPetugasByLoad(dinasId: string): Promise<string | null> {
  ...
  const indexed = officers.map((o, i) => ({ id: o.id, count: taskCounts[i] }));
